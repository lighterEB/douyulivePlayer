...
class Player:
    ...
    def parse_video(self, uRl):
        self.cMedia = self.instance.media_new_location(uRl)
        self.media.set_media(self.cMedia)
    # 播放 成功返回0，失败返回-1
    ...
    # 绑定句柄
    def set_handleWin(self, winid):
        if platform.system() == "Windows":
            self.media.set_hwnd(winid)
        else:
            self.media.set_xwindow(winid)
...
def my_callback(event):
    print("call:", play.get_time())

if __name__ == "__main__":
    play = Player()
    play.add_callback(vlc.EventType.MediaPlayerTimeChanged, my_callback)
    play.play("C:\\Users\\tianshan\\Downloads\\video.mp4")
